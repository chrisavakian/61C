...
void vector_set(vector_t *v, size_t loc, int value) {
    ...
    if (loc > v->size - 1) {
        //Reallocate an extra size of int in place for the new value
        //NOTE: if loc is more than one index higher this will NOT work
        ...
        //unset locations SHOULD CONTAIN A VALUE OF -1
        //was litteraly written out for me and I didn't understand
        for (int i = v->size; i < loc; i++) {
            v->data[i] = 0;
        } 

        //Damnit forgot to update the size
        v->size = loc + 1;
